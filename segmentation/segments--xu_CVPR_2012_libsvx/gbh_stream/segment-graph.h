...
void segment_graph(universe* mess, vector<edge>* edges_remain, 
		edge* edges, int num_edges, float c, int level) {
...
void segment_graph_region(universe* mess, vector<edge>* edges_remain, 
	...
	// new a vector containing remain edges
	edges_remain->clear();
	// sort edges by weight
	sort(edges_region->begin(), edges_region->end());
	// for each edge, in non-decreasing weight order...
	for (int i; i < ((int) edges_region->size()); i++) {
