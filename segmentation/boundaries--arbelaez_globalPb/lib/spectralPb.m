...
path = SvmSegm_paths();
system([fullfile(path.repos_path, '../../utils/segmentation/boundaries--arbelaez_globalPb/lib/segment'), sprintf(' -image %s -smatrixfile %s -readlattice true -latticefile %s -dthresh 5', imgFile, simFile, latFile)]);
delete(latFile);
delete(imgFile);
