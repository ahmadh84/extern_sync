function [phi,frames,numIter,speed_grad] = evolve_height_function_N(img, time_step, num_iterations, speed_type, display_interval, boundary_speed_interval, numSuperPixels, seeds, Pb)
    ...
    if (strcmp(speed_type,'superpixels'))
        ...
        if (~exist('Pb', 'var') || isempty(Pb))
            ...
            [speed_grad,speed_grad_x,speed_grad_y] = get_speed_based_on_gradient(Pb,[],normSigma,phi);
        end
        speed = speed_grad;
        ...
        if (strcmp(speed_type,'superpixels'))
            if (i < 20)
                ...
                if (~exist('Pb', 'var') || isempty(Pb))
                    ...
                    speed = get_speed_based_on_gradient_pb(Pb,1,normSigma,phi,speed_grad_extended,speed_grad_x_extended,speed_grad_y_extended);
                end
            end
        end
