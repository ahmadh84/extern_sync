...
typedef struct { int c, r, w, h; float s; vectorf edgeArr; } Box;
...
public:
  ...
  void generate( Boxes &boxes, arrayf &E, arrayf &O, arrayf &V, 
                 mxArray*& segIdptr, mxArray*& segCptr, mxArray*& segRptr );

  int getNumEdgeGrps() const { return _segCnt; }
  void writeBoxIm( Box &box, arrayf &V, const int& idx );
private:
  ...
  int *_segR, *_segC;             // segment lower-right pixel
  ...
  // helper routines
  void clusterEdges( arrayf &E, arrayf &O, arrayf &V, mxArray*& segIdptr, 
                     mxArray*& segCptr, mxArray*& segRptr );
  void prepDataStructs( arrayf &E );
  void scoreAllBoxes( Boxes &boxes );
  int scoreBox( Box &box );
  void refineBox( Box &box );
  void drawBox( Box &box, arrayf &E, arrayf &V );
  void collectEdgeVals( Box &box );
};
...
void EdgeBoxGenerator::generate( Boxes &boxes, arrayf &E, arrayf &O, arrayf &V, 
                                 mxArray*& segIdptr, mxArray*& segCptr, mxArray*& segRptr )
{
  clusterEdges(E,O,V,segIdptr,segCptr,segRptr); prepDataStructs(E); scoreAllBoxes(boxes);

//  for (int i = 0; i < (int) boxes.size(); ++i)
    drawBox(boxes[0], E, V );
}

void EdgeBoxGenerator::clusterEdges( arrayf &E, arrayf &O, arrayf &V, mxArray*& segIdptr, 
                                     mxArray*& segCptr, mxArray*& segRptr )
{
  ...
  const int ds[2] = {h,w};
  segIdptr = mxCreateNumericArray(2,ds,mxINT32_CLASS,mxREAL);
  _segIds._x = (int*) mxGetData(segIdptr); _segIds._h=h; _segIds._w=w;
  //_segIds.init(h,w); 
  _segCnt=1;
  ...
  const int ds2[1] = {_segCnt};
  segCptr = mxCreateNumericArray(1, ds2, mxINT32_CLASS, mxREAL);
  _segC = (int*) mxGetData(segCptr);
  segRptr = mxCreateNumericArray(1, ds2, mxINT32_CLASS, mxREAL);
  _segR = (int*) mxGetData(segRptr);
...
int EdgeBoxGenerator::scoreBox( Box &box )
{
  ...
  if( box.s<_minScore ) { box.s=0; return -1; }
...
void EdgeBoxGenerator::drawBox( Box &box, arrayf &E, arrayf &V )
{
  ...
  for (c=0; c<w; ++c) {
      for (r=0; r<h; ++r) {
          ...
          e = E.val(c,r);
          if (_sDone._x[i]==sId) {
              o = _sWts._x[_sMap._x[i]];
          } else {
              if (_segC[i] >= c0 && _segC[i] <= c1 && 
                  _segR[i] >= r0 && _segR[i] <= r1)
                  o = 0;
              else
                  o = 1;
          }
          V.val(c+w*0, r) = 1 - e + e*o;
          V.val(c+w*1, r) = 1 - o;   // 1 - e*o
          V.val(c+w*2, r) = 1 - e;
      }
  }

  //mexPrintf("[%d, %d] - %d\n", smalli, largei, _segCnt);
...
void EdgeBoxGenerator::writeBoxIm( Box &box, arrayf &V, const int& idx )
{
  // score box and draw color coded edges (red=out, green=in)
  int i, c, r;
  float o;
  ...
  for (c=0; c<w; ++c) {
      for (r=0; r<h; ++r) {
          ...
          V.val(c+w*idx, r) = o;   // 1 - e*o
      }
  }
}

void EdgeBoxGenerator::collectEdgeVals( Box &box )
{  
  float o;
  ...
  for( int s=0; s<scNum; s++ ) {
    ...
    for( a=0; a<2*arRad+1; a++ ) {
      ...
      for( c=0; c<w-bw+kc; c+=kc ) for( r=0; r<h-bh+kr; r+=kr ) {
        Box b; b.r=r; b.c=c; b.h=bh; b.w=bw; b.edgeArr.resize(_segCnt);
        boxes.push_back(b);
      }
    }
  }
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  mxArray *segIdptr, *segCptr, *segRptr, *edgeArrptr;
  ...
  int m = edgeBoxGen.getNumEdgeGrps();
  const int ds2[2] = {m,n};
  edgeArrptr = mxCreateNumericArray(2,ds2,mxSINGLE_CLASS,mxREAL);
  float *edgeArr = (float*)mxGetData(edgeArrptr);


  arrayf W; if( nl>1 ) {
    const int ds[3] = {h,w,n};
    pl[1] = mxCreateNumericArray(3,ds,mxSINGLE_CLASS,mxREAL);
    W._x = (float*) mxGetData(pl[1]); W._h=h; W._w=w;
  }
  ...
  for(int i=0; i<n; i++) {
    ...
    bbs[ i + 4*n ] = boxes[i].s;
    memcpy(&edgeArr[m*i], &boxes[i].edgeArr[0], m*sizeof(float));
    edgeBoxGen.writeBoxIm( boxes[i], W, i );
  }

  // return back to the caller function, otherwise delete memory
  if (nl > 2)
      pl[2] = segIdptr;
  else
      mxDestroyArray(segIdptr);
  
  if (nl > 3)
      pl[3] = edgeArrptr;
  else
      mxDestroyArray(edgeArrptr);

  if (nl > 4)
      pl[4] = segRptr;
  else
      mxDestroyArray(segRptr);
  
  if (nl > 5)
      pl[5] = segCptr;
  else
      mxDestroyArray(segCptr);
}
